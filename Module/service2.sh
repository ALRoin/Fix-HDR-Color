...
(
    ...
    is_hdr_active() {
        dumpsys SurfaceFlinger 2>/dev/null | awk '
            /------------HWC----------------/ { inrange=1; next }
            /----------Color Modes---------/  { inrange=0 }
            inrange && /dataspace:0x(11c60000|12060000|09c60000|0a060000)/ { found=1; exit }
            END { exit !found }
        '
    }
    ...
    reload_intervals_if_changed() {
        ...
        if [ "$new_mtime" != "$INTERVAL_MTIME" ]; then
            ...
            log_msg "INTERVALS RELOADED: FOREGROUND_INTERVAL=$FOREGROUND_INTERVAL HDR_CHECK_INTERVAL=$HDR_CHECK_INTERVAL SCREENOFF_INTERVAL=$SCREENOFF_INTERVAL"
        fi
    }
...
EOF
        ...
        log_msg "TARGET APP FOREGROUND: $MATCHED_PKG -> Monitoring for HDR playback."

        while [ "$MATCH_FOUND" -eq 1 ]; do
            if [ -f "$TOGGLE_FILE" ]; then
                log_msg "Service STOPPED via disable_service flag."
                exit 0
            fi
            ...
            if ! is_screen_on; then
                log_msg "SCREEN OFF (during HDR monitoring): Returning to main loop."
                break
            fi
            ...
            if is_hdr_active; then
                if [ "$CURRENT_STATE" -eq 0 ]; then
                    ...
                    CURRENT_STATE=1
                    log_msg "HDR ACTIVE ($MATCHED_PKG): HW Overlays Disabled."
                fi
            else
                if [ "$CURRENT_STATE" -eq 1 ]; then
                    service call SurfaceFlinger 1008 i32 0 > /dev/null 2>&1
                    CURRENT_STATE=0
                    log_msg "HDR INACTIVE ($MATCHED_PKG): Restored HW Overlays."
                fi
            fi

            reload_intervals_if_changed
            sleep "$HDR_CHECK_INTERVAL"
